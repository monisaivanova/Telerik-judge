...
const test = [
    ...
    'x x x x x x x x x x x x x x - x x x x x x x - x x x - x - x x x x x x x x x x x x x x x x - x x x - x x x x x x x x - x x x x x x x x x x x x - x x x x x x x - x x x x x x x - x x x x x x x x x x x e',  
];
...
const iterate = (row, col, currentPath) => {
    if (matrix[row][col] === 'e') {
    ...
    for (const move of moves) {
        const nextRow = Number(row) + move[0];
        const nextCol = Number(col) + move[1];

        if (nextRow >= 0 && nextRow < n && nextCol >= 0 && nextCol < n) {
            const symbol = matrix[nextRow][nextCol];
            if (symbol === '-') {
                matrix[nextRow][nextCol] = 'x';
                iterate(nextRow, nextCol, currentPath + 1);
...
matrix[startRow][startCol] = 'ne';
iterate(startRow, startCol, 0, 'ne');
